  ...
  <div class="list-wrapper" ref="TableHeadWrapper">
    ...
    <div class="table list-table-head" :class="{tableHeadTop:scrolled}">
      <div class="table-tr">
        ...
        <div class="table-th td-date sorting sorting_asc"  @click="sorting = 'time'">时间</div>
        <div class="table-th td-location">来源</div>
        <!-- <div class="table-th td-emotion">情绪</div> -->
        <!-- <div class="table-th td-sensitive sorting sorting_asc" @click="sorting = 'risk'">风险度</div> -->
        <!-- <div class="table-th td-recommend sorting sorting_asc" @click="sorting = 'recommend'">推荐</div> -->
        ...
        <div class="table-tr" v-for="(item, idx) in dispValues" :key="item.id">
          ...
          <div class="table-tr-row">
            ...
            <div class="table-td td-title" v-if='isevent'>
              <router-link class="title-link" :to="{name:'Event/Detail/Overall', params: {eventId: item.id}}">
                {{ item.title }}
              </router-link>
            </div>
            ...
            <div class="table-td sorting td-date">{{ item.date }}</div>
            <div class="table-td td-location">{{ item.source }}</div>
            <!-- <div class="table-td td-emotion">{{ item.emotion | toEmotion }}</div>
            <div class="table-td td-sensitive sorting">{{ Math.max(item.risk, 0) }}</div>
            <div class="table-td td-recommend sorting">{{ Math.max(item.recommend, 0) }}</div> -->
            <!-- <div class="table-td td-source" v-if="!isevent">
              <span v-if="item.datatag === 'event'">
                <a class="event-link" :href="'#/event/detail/' + item.id">演化分析</a>
              </span>
          ...
          <!-- 更多列表 -->
          <div class="from-source" v-if="toggleSource == item.id" v-for="(subitem, idx) in sourceMore" :key="idx">
  ...
  watch: {
    dispDatas: function (datas) {
      let xs = _.map(_.uniqBy(datas, 'id'), item => {
        ...
        //   item.url = '#/event/detail/' + item.id;
        // }
        // // Location去掉”中国“前缀
        // if (item.location.startsWith('中国')) {
      ...
      if (this.sorting) {
        this.dispValues = _.orderBy(xs, this.sorting, 'desc');
      } else {
        this.dispValues = xs;
      }
    },
    sorting: function () {
      this.dispValues = _.orderBy(this.dispValues, this.sorting, 'desc');
    },
  },
  methods: {
    handleScroll () {
    ...
    setFieldValueAs (esIndex, id, field, value) {
      ...
      axios.post('/api/source/setFieldValueAs', params.toString()).then(response => {
